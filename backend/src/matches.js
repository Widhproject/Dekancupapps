...
// ============================================================
// FOTO/DOKUMENTASI PERTANDINGAN — disimpan di backend/uploads/match-photos
// dan disajikan lewat static route /uploads (lihat server.js), sama seperti
// pola upload file formulir pendaftaran (registrations.js).
// ============================================================
...
const uploadPhoto = multer({
  ...
  limits: { fileSize: 8 * 1024 * 1024 }, // maks 8 MB per foto
...
// Tambah foto/dokumentasi pertandingan (upload file langsung dari device panitia).
router.post('/:id/photos', requireAuth, requireAdmin, uploadMatchPhoto, (req, res) => {
  const match = db.matches.find((m) => m.id === req.params.id);
  if (!match) return res.status(404).json({ message: 'Pertandingan tidak ditemukan' });
  if (!canManageSport(req.user, match.sport_type)) {
    if (req.file) fs.unlink(path.join(matchPhotosDir, req.file.filename), () => {});
    return res.status(403).json({ message: `Akun Anda tidak diizinkan mengelola cabor ${match.sport_type}` });
  }
